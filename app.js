...
function setStatus(texto) {
  document.getElementById("last-updated").textContent = texto;
}
...
async function cargarDatos() {
  ...
  try {
    ...
    mostrarToast("No se pudo conectar al servidor. Intentá recargar la página.", "error");
  }
  render();
}
...
const save = async () => {
  ...
  try {
    ...
    if (!res.ok) throw new Error("HTTP " + res.status);
    const now = new Date();
    setStatus("Guardado " + now.toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" }));
  } catch (e) {
    console.error(e);
    setStatus("⚠ Error al guardar");
    mostrarToast("Error al guardar. Los cambios no se persistieron.", "error");
  } finally {
...
function addTempCylinder() {
  ...
  if (!gas || !qty) { mostrarToast("Elegí el gas y la cantidad.", "warn"); return; }
  if (rot > qty) { mostrarToast("La baja rotación no puede ser mayor a la cantidad total.", "warn"); return; }
...
function renderNuevosCilindros() {
  ...
  div.innerHTML = nuevosCilindros.map((g, i) => `
    <div class="cyl-row">
      ...
      <button class="btn btn-ghost btn-sm" onclick="removeTempCylinder(${i})" aria-label="Eliminar ${g.gas}">🗑</button>
    </div>`).join("");
}

function addClient() {
  const n = document.getElementById("add-nombre").value.trim();
  if (!n) { mostrarToast("Falta el nombre del cliente.", "warn"); document.getElementById("add-nombre").focus(); return; }
  const pago = document.getElementById("add-pago").value;
  const estado = document.getElementById("add-estado").value;
  const c = {
    id: Date.now(), nombre: n, pagoHasta: pago, estado,
    notas: document.getElementById("add-notas").value.trim(),
    cilindros: [...nuevosCilindros], historial: []
  };
  if (estado === "Pagado" && pago) c.historial.push({ mes: pago, nota: "Alta" });
  clientes.push(c);
  save(); closeAdd(); render();
  mostrarToast(`Cliente "${n}" agregado.`);
}
...
const saveNotasDebounced = debounce(() => {
...
function marcarPagadoHoy() {
  const c = clientes[selected];
  const mes = mesActual();
  c.estado = "Pagado"; c.pagoHasta = mes;
  if (!c.historial) c.historial = [];
  ...
  save(); render(); openDetalle(selected);
  mostrarToast("✔ Pago del mes registrado.");
}
...
function addHistorial() {
  ...
  if (!mes) { mostrarToast("Seleccioná el mes.", "warn"); return; }
...
function renderHistorial() {
  const hist = clientes[selected].historial || [];
  ...
  div.innerHTML = hist.map((h, i) => `
    <div class="hist-row">
      ...
      <button class="btn btn-ghost btn-sm" style="padding:2px 6px;font-size:11px"
        onclick="removeHistorial(${i})" aria-label="Eliminar registro ${h.mes}">✕</button>
    </div>`).join("");
}
...
function renderCylinders() {
  ...
  div.innerHTML = cyls.map((g, i) => `
    <div class="cyl-row">
      <select onchange="updateCyl(${i},'gas',this.value)" aria-label="Tipo de gas">
        ${GASES.map(gas => `<option value="${gas}" ${g.gas === gas ? "selected" : ""}>${gas}</option>`).join("")}
      </select>
      <span style="font-size:11px;color:var(--text3)">Cant.</span>
      <input type="number" value="${g.cantidad}" min="0"
        onchange="updateCyl(${i},'cantidad',this.value)" aria-label="Cantidad">
      <span style="font-size:11px;color:var(--text3)">Baja rot.</span>
      <input type="number" value="${g.bajaRotacion || 0}" min="0"
        onchange="updateCyl(${i},'bajaRotacion',this.value)" aria-label="Baja rotación">
      <button class="btn btn-ghost btn-sm" onclick="removeCylinder(${i})"
        aria-label="Eliminar cilindro ${g.gas}">🗑</button>
    </div>`).join("");
}
...
function doDelete() {
  const nombre = clientes[selected].nombre;
...
function renderReportes() {
  ...
  tbody.innerHTML = gasKeys.length
    ? gasKeys.map(g => {
        const r = porGas[g];
        return `<tr><td>${g}</td><td>${r.total}</td><td>${r.bajaRotacion}</td><td>${r.clientes.size}</td></tr>`;
      }).join("")
    : '<tr><td colspan="4" style="text-align:center;color:var(--text3);font-style:italic;padding:14px">Sin cilindros cargados todavía.</td></tr>';
}

// ─── Ayuda ─────────────────────────────────────────────────────────────────

function openAyuda() { document.getElementById("overlay-ayuda").style.display = "flex"; }
function closeAyuda() { document.getElementById("overlay-ayuda").style.display = "none"; }
...
async function exportExcel() {
  ...
  clientes.forEach(c => {
    ...
    const row = wsClientes.addRow({
      nombre: c.nombre, estado: c.estado, pago: c.pagoHasta || "",
      meses: d === null ? "-" : d < 0 ? "Al día" : d,
      total: totalTubos(c), rot: totalBajaRotacion(c), notas: c.notas || ""
    });
    let fill = null;
...
function render() {
  ...
  let lista = clientes.map((c, i) => ({ c, i })).filter(({ c }) => {
    ...
    const d = mesesDesde(c.pagoHasta);
    if (filtroActivo === "urgente") { if (!(d !== null && d >= 3)) return false; }
    else if (filtroActivo === "porvencer") { if (!(d !== null && d >= 0 && d <= 2)) return false; }
    else if (filtroActivo !== "todos" && c.estado !== filtroActivo) return false;
  ...
  if (!lista.length) {
    panel.innerHTML = `
      <div class="empty-state">
        ...
        <p>${q || filtroActivo !== "todos"
          ? "No hay clientes que coincidan con ese filtro."
          : "Agregá tu primer cliente con el botón de arriba."}</p>
      </div>`;
    return;
  }
      ...
      <div class="card ${cls}">
        <div style="flex:1;min-width:0">
          ...
          <div class="card-pills">
            ...
            ${urgente
              ? `<span class="pill pill-red">⚠️ ${d} meses sin pagar</span>`
              : (d !== null && d >= 0 && d <= 2)
                ? `<span class="pill pill-orange">🔜 ${mesesTexto(d)}</span>`
                : `<span class="pill pill-gray">📅 ${mesesTexto(d)}</span>`}
        ...
        <div class="card-right">
          <button class="btn btn-sm" onclick="openDetalle(${i})"
            aria-label="Ver detalle de ${c.nombre}">Ver →</button>
        </div>
      </div>`;
  }).join("");
}
...
document.addEventListener("keydown", (e) => {
  ...
  const overlays = ["overlay-detalle", "overlay-add", "overlay-reportes", "overlay-ayuda"];
  for (const id of overlays) {
    const el = document.getElementById(id);
    if (el && el.style.display === "flex") { el.style.display = "none"; return; }
  }
  const confirmBox = document.getElementById("confirm-box");
  if (confirmBox && confirmBox.style.display === "flex") closeConfirm();
});

// Cerrar con click fuera del modal
["overlay-detalle", "overlay-add", "overlay-reportes", "overlay-ayuda"].forEach(id => {
